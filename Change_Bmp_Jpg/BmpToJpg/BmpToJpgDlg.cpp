...
BEGIN_MESSAGE_MAP(CBmpToJpgDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_CHOOSE, &CBmpToJpgDlg::OnBnClickedButtonChoose)
	ON_BN_CLICKED(IDC_BUTTON_CHOOSEOUT, &CBmpToJpgDlg::OnBnClickedButtonChooseout)
	//}}AFX_MSG_MAP
...
BOOL CBmpToJpgDlg::OnInitDialog()
{
	...
	if (pSysMenu != NULL)
	{
		...
		beNameVaild = strAboutMenu.LoadString(IDS_ABOUTBOX);
		ASSERT(beNameVaild);
	...
	m_boxChangeMode.AddString("BmpToJpg"); 
	m_boxChangeMode.AddString("JpgToBmp"); 
...
void CBmpToJpgDlg::OnBnClickedButtonChange()
{
	...
	if (iSelect == 1 && strCheck1 == _T("JPG") && strCheck2 == _T("BMP"))
	{
		iRet = m_jpgToBmp.LoadJpegFile(strInPath.GetBuffer(0), strOutPath.GetBuffer(0));
	}
	else if (iSelect == 0 && strCheck1 == _T("BMP") && strCheck2 == _T("JPG"))
	{
		BOOL isChange;
		pFunc = (CHANGE*)::GetProcAddress(hInst, "BmpToJpg");
		isChange = pFunc(strInPath.GetBuffer(0), strOutPath.GetBuffer(0));
	...
	{
		AfxMessageBox(_T("Please Check The Format of Files!"));
		return;
	}
...
void CBmpToJpgDlg::OnBnClickedButtonChoose()
{
...
void CBmpToJpgDlg::OnBnClickedButtonChooseout()
{
	CFileDialog dlg(TRUE, _T("bin"), NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
		_T(" Files (*.jpg)|*.jpg|(*.bmp)|*.bmp|All Files (*.*)|*.*||"), this);
